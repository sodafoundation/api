...
FILE_LVM_DIR=$OPT_DIR/nfsnative
...
cat > $OPENSDS_DRIVER_CONFIG_DIR/nfsnative.yaml << OPENSDS_FILE_CONFIG_DOC
tgtBindIp: $HOST_IP
tgtConfDir: /etc/tgt/conf.d
pool:
...
driver_name = nfsnative
config_path = /etc/opensds/driver/nfsnative.yaml
...
osds::lvm::clean_volume_group_for_file() {
    ...
    if [[ -z "$(sudo lvs --noheadings -o lv_name $fvg 2>/dev/null)" ]]; then
        osds::lvm::clean_backing_file $DATA_DIR/$fvg$BACKING_FILE_SUFFIX
    fi
}
...
osds::lvm::cleanup(){
    ...
    osds::lvm::clean_volume_group $FILE_DEFAULT_VOLUME_GROUP_NAME
    osds::lvm::clean_lvm_filter
    local nvmevg=$NVME_VOLUME_GROUP_NAME
    if vgs $nvmevg ; then
