...
func (plugin *ManilaPlugin) UpdateShare(shrID string, name string) (string, error) {
	shareService, err := plugin.getShareService()
	if err != nil {
		log.Println("Cannot access file share service:", err)
		return "", err
	...
	requestBody.Name = name
	body := share.CreateBody{requestBody}
	share, err := shareService.Update(shrID, &body)
	if err != nil {
		log.Println("Cannot update file share:", err)
		return "", err
	}

	a, _ := json.Marshal(share)
	result := string(a)
	log.Println("Update file share success, dls =", result)
	return result, nil
}
