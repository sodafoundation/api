...
iscsiadm -m discovery -t sendtargets -p $TARGETIP >$TARGETLOG
...
cat $TARGETLOG |while read line
  do
     ...
     echo $a
      ...
      echo $target
  #login
    iscsiadm -m node –T $TARGETIP，$target -l >$LOGINLOG
   done
...
if [ `grep -c "Disk /dev/sdb: 1 GiB" $DISKLOG` -eq 1 ];then
...
iscsiadm -m node –T $target -p $TARGETIP -u
echo >$SESSIONLOG
iscsiadm -m session >>$SESSIONLOG
