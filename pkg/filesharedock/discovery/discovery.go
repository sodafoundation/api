...
func (pdd *provisionDockDiscoverer) Init() error {
	...
	for _, v := range CONF.FileShareEnabledBackends {
		b := bm[v]
		log.Error(" ", b)
		if b.Name == "" {
			continue
		...
		dck := &model.DockSpec{
			...
			Endpoint:    CONF.OsdsfileDock.ApiEndpoint,
			NodeId:      host,
			Type:        model.DockTypeProvioner,
			Metadata:    map[string]string{"HostReplicationDriver": CONF.OsdsfileDock.HostBasedReplicationDriver},
		}
