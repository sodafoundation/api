...
import (
	...
	_ "github.com/opensds/opensds/pkg/context"
	_ "github.com/opensds/opensds/pkg/db"
	"github.com/opensds/opensds/pkg/filesharedock/discovery"
	_ "github.com/opensds/opensds/pkg/model"
...
func (ds *dockServer) DeleteFileShare(ctx context.Context, opt *pb.DeleteFileShareOpts) (*pb.GenericResponse, error) {
	...
	if _, err := ds.Driver.DeleteFileShare(opt); err != nil {
		log.Error("error occurred in dock module when delete file share:", err)
		//Update in DB if there is any error while deleting
		//db.UpdateVolumeStatus(ctx, db.C, fileshareId, model.FileShareError)
		return pb.GenericResponseError(err), err
	}
	...
	//db.C.DeleteVolume(ctx, fileshareId)
	// TODO: maybe need to update status in DB.
