...
import (
	...
	_ "github.com/opensds/opensds/pkg/utils"
	"golang.org/x/net/context"
	"google.golang.org/grpc"
)

func FileShareNewController(port string) *Controller {
	fileshareCtrl := fileshare.FileShareNewController()
	return &Controller{
...
// DeleteVolume implements pb.ControllerServer.DeleteFileShare
func (c *Controller) DeleteFileShare(contx context.Context, opt *pb.DeleteFileShareOpts) (*pb.GenericResponse, error) {
	...
	if err != nil {
		...
		db.UpdateVolumeStatus(ctx, db.C, opt.Id, model.FileShareErrorDeleting)
		return pb.GenericResponseError(err), err
	}
