...
func init() {
	...
	beego.Router("/api", &versionPortal, "get:ListVersions")
	beego.Router("/api/:apiVersion", &versionPortal, "get:GetVersion")
}
...
func TestGetVersion(t *testing.T) {
	r, _ := http.NewRequest("GET", "/api/v1alpha", nil)
	w := httptest.NewRecorder()
	beego.BeeApp.Handlers.ServeHTTP(w, r)
...
func TestGetVersionWithInvalidAPIVersion(t *testing.T) {
	r, _ := http.NewRequest("GET", "/api/InvalidAPIVersion", nil)
	w := httptest.NewRecorder()
	beego.BeeApp.Handlers.ServeHTTP(w, r)
