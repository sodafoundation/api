...
func (this *VolumeGroupPortal) ListVolumeGroups() {
	if !policy.Authorize(this.Ctx, "volumeGroup:list") {
		return
	}
	...
	if err != nil {
		this.ErrorHandle("List volumes failed", model.ErrorBadRequest, err)
		return
	}

	result, err := ListVolumeGroupsWithFilter(c.GetContext(this.Ctx), m)
	if err != nil {
		this.ErrorHandle("List volumes failed", model.ErrorBadRequest, err)
		return
	}
	...
	body, err := json.Marshal(result)
	if err != nil {
		this.ErrorHandle("Marshal volumes listed result failed", model.ErrorInternalServer, err)
		return
	}
	this.SuccessHandle(StatusOK, body)
	return
