...
import (
	...
	"github.com/opensds/opensds/contrib/drivers/volume/lvm/targets"
...
func NewDriver(properties config.BackendProperties) factory.Driver {
	return &Driver{properties: properties}
}
...
func (d *Driver) Setup() error {
	...
	p := defaultConfPath
	if len(d.properties.ConfigPath) != 0 {
		p = d.properties.ConfigPath
	}
...
func (d *Driver) AttachSnapshot(snapshotId string, lvsPath string) (string, *model.ConnectionInfo, error) {
	...
	createOpt := &pb.CreateSnapshotAttachmentOpts{
		...
		AccessProtocol: constants.ISCSIProtocol,
	}
